...
public class CommandTempo implements CommandExecutor
{
    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String commandLabel, final String[] args) {
        ...
        if (sender instanceof Player && sender.hasPermission("style.tempo") && command.getName().equalsIgnoreCase("tempo") && args.length != 0) {
            if (args[0].equalsIgnoreCase("noite")) {
                ...
                Bukkit.broadcastMessage(ChatColor.GREEN + "O tempo foi alterado para" + ChatColor.WHITE + " Noite " + ChatColor.GREEN + "por " + ChatColor.WHITE + sender.getName());
                return true;
            }
            if (args[0].equalsIgnoreCase("dia")) {
                player.getWorld().setTime(0L);
                sender.sendMessage(ChatColor.GREEN + "O tempo foi alterado para: " + ChatColor.DARK_GREEN + "Dia");
                Bukkit.broadcastMessage(ChatColor.GREEN + "O tempo foi alterado para" + ChatColor.WHITE + " Dia " + ChatColor.GREEN + "por " + ChatColor.WHITE + sender.getName());
                return true;
            }
        }
        return false;
