		...
		if (cmd.getName().equalsIgnoreCase("tp")) {
			...
			if (args.length == 0) {
				...
				if (p.hasPermission("style.tp") || p.isOp()) {
					...
					p.teleport(tpLoc);
					p.sendMessage(ChatColor.YELLOW + "Teleportado para " + targetPlayer.getDisplayName()
							+ ChatColor.YELLOW + ".");
				} else {
				...
				if (p.hasPermission("style.tp.otros") || p.isOp()) {
					...
					if (targetPlayer == null || targetPlayer2 == null) {
						...
						p.sendMessage(ChatColor.YELLOW + "Voce foi teleportado " + targetPlayer.getDisplayName()
								+ ChatColor.YELLOW + " para " + targetPlayer2.getDisplayName() + ChatColor.YELLOW
								+ ".");
					}
				} else {
					Permissions.semPermiassao(p);
