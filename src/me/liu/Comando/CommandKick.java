            ...
            if (p.hasPermission("style.kick") || p.isOp()) {
                if (args.length == 0) {
                    ...
                    if (targetPlayer == null) {
                        ...
                        Bukkit.broadcastMessage(ChatColor.RED + targetPlayer.getDisplayName() + "§e foi kickado por §c" + p.getDisplayName() + ".");
                    }
                }
                else if (args.length > 1) {
                    final Player targetPlayer = p.getServer().getPlayer(args[0]);
                    if (targetPlayer == null) {
                        ...
                        targetPlayer.kickPlayer("§eVoce foi kickado pelo jogador §c" + p.getDisplayName() + " §epor §c§o§l> " + message);
                        Bukkit.broadcastMessage(ChatColor.RED + targetPlayer.getDisplayName() + " §eFoi kickado por §c" + p.getDisplayName() + " §epor " + message);
                    }
                }
            }
            else {
