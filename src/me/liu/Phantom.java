...
public class Phantom implements Listener, CommandExecutor
{
    ...
    public void onInteractPhantom(final PlayerInteractEvent event) {
        ...
        if (event.getPlayer().hasPermission("style.phantom") && event.getAction().name().contains("RIGHT") && event.getPlayer().getItemInHand().getType() == Material.FEATHER && Main.phantom.contains(event.getPlayer().getName())) {
