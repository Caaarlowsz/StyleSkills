...
import me.liu.Gui.*;
import me.liu.Comando.*;
import me.liu.Listeners.*;
...
public class Main extends JavaPlugin implements Listener
{
    ...
    public Main() {
        this.ShopDeItens = (Listener)new CommandShop(this);
    ...
    public void onEnable() {
        ...
        this.getConfig().options().copyDefaults(true);
        this.saveConfig();
        final int interval = this.getConfig().getInt("timehour") * 60 * 60 + this.getConfig().getInt("timemin") * 60 + this.getConfig().getInt("timesec");
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        if (cmd.getName().equalsIgnoreCase("report")) {
            if (args.length < 1) {
                ...
                this.getLogger().info(String.valueOf(sender.getName()) + " Reportou " + ReportName + " Por \"" + ReportReson + "\"");
                sender.sendMessage("§cJogador reportado com sucesso se ouver moderadores online verao seu report!");
                Player[] onlinePlayers;
                for (int length = (onlinePlayers = this.getServer().getOnlinePlayers()).length, j = 0; j < length; ++j) {
                    final Player player = onlinePlayers[j];
                    if (player.hasPermission("derp.report.receber")) {
                        if (ReportReson != "") {
                            player.sendMessage("§a" + sender.getName() + "§c Reportou §a" + ReportName + "§a Por \"§a" + ReportReson + "§a\"");
                        }
                        else {
                            player.sendMessage("§a" + sender.getName() + "§c Reportou §a" + ReportName + "§a Sem Razao!");
                        }
                    }
                }
            }
    ...
    public void onDeath(final PlayerDeathEvent event) {
        ...
        if (p.getKiller() instanceof Player) {
            ...
            killer.sendMessage(ChatColor.GREEN + "[StylePvP] " + ChatColor.GOLD + "Voce ganhou " + r + " por matar " + p.getName() + ".");
